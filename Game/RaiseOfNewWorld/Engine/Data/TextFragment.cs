...
public abstract class TextFragment
{
    private sealed class SimpleText : TextFragment
    {
        ...
        {
            var label = DefaultLayout(new Label
            {
                Text = _text
            }, next);
    ...
    private sealed class CompountFragment : TextFragment
    {
        ...
        public CompountFragment(IEnumerable<TextFragment> fragments) => _fragments = fragments.ToArray();

        protected override View Render(View view, View? next)
        {
            // ReSharper disable once LoopCanBeConvertedToQuery
            foreach (var fragment in _fragments) next = fragment.Render(view, next);
    ...
    public static TextFragment Create(string text, Action<Label>? attributeBuilder)
        => new SimpleText(text, attributeBuilder);
    ...
    public void Render(View view) => Render(view, null);
