...
{
    public static Func<T, TR?> ToFunc<T, TR>(this Action<T> action)
    {
        return x =>
        {
            action(x);
    ...
    public static Func<T, TR> ToFunc<T, TR>(this Action<T> action, TR result)
    {
        return x =>
        {
            action(x);
    ...
    public static bool IsSuccess<T>(Action<T?>? action) => IsSuccess(action, default);
    ...
    /// <param name="pause">The amount of time to pause between retries</param>
    /// <param name="retryOnException">
    ///     A predicate that will validate if the action should continue retrying after the given
    ///     has been thrown
    /// </param>
    /// <returns>An action that wraps he given <see cref="action" /></returns>
