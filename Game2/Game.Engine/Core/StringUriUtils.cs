...
public sealed class StringUriUtils
{
    ...
    /// </remarks>
    public static readonly char[] AcceptableSeperatorChars = { '/', '\\' };
    ...
    public static string NormalizePath(string path, int startIndex, int count) =>
        NormalizePath(path, startIndex, count, AcceptableSeperatorChars, false);

    private static bool CharArrayContains(char[] array, char c)
    {
        return array.Any(t => t == c);
    }
    ...
    public static string BuildQuery(NameValueCollection nameValueCollection) => BuildQuery(
        nameValueCollection.EnumerateEntrys(),
        PredicateUtils<KeyValuePair<string, string>>.AlwaysTrue);
    ...
    public static string BuildQuery(IEnumerable<KeyValuePair<string, string>> pairs) =>
        BuildQuery(pairs, PredicateUtils<KeyValuePair<string, string>>.AlwaysTrue);

    public static string BuildQuery(IEnumerable<(string, string)> pairs)
    {
        return BuildQuery(pairs.Select(t => KeyValuePair.Create(t.Item1, t.Item2)), PredicateUtils<KeyValuePair<string, string>>.AlwaysTrue);
    }
    ...
    public static string RemoveQuery(string uri) => uri.Left(PredicateUtils.ObjectEquals('?').Not());
