...
namespace Tauron.Application.CelloManager.Logic.RefillPrinter.Rule
{
    ...
    public class MailHelper
    {
        public static bool MailOrder(FlowDocument document, string email, string dns, ISettings settings)
        {
            ...
            MailMessage message = GetMessage(email, document);
        ...
        public static MailMessage GetMessage(string email, FlowDocument doc)
        {
            try
            {
                ...
                msg.From = new MailAddress("Cello Manager", "cello@blueprint.de");
                msg.To.Add(new MailAddress(email));
                byte[] pdfDocument = ConvertToPdf(doc);

                msg.Body = UIResources.EmailerMessageBody;
                msg.Attachments.Add(new Attachment(new MemoryStream(pdfDocument), "Order.pdf", "application/pdf"));
        ...
        public static bool SendMail(MailMessage message, string server)
        {
            try
            {
                using (var smclient = new SmtpClient(server))
                {
